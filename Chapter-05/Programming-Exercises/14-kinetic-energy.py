...
def kinetic_energy(mass, velocity):
	return FACTOR * mass * velocity * velocity
def get_mass():
	return float(input("What is the object's mass in kg? "))
def get_velocity():
	return float(input("What is the object's velocity in m/s? "))
def main():
	print(format(kinetic_energy(get_mass(), get_velocity()), '.2f'))
main()
